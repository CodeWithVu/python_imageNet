...
def create_model(img_shape, class_count, lr=0.0008):
    """
    Tạo model ResNet50 - Tương thích với Keras 3.x (TensorFlow 2.16+)
    
    Các thay đổi quan trọng cho Keras 3.x:
    1. Sử dụng Functional API với tf.keras.Input()
    2. Thêm preprocessing layer cho ResNet
    ...
    # QUAN TRỌNG: Thêm preprocessing layer cho ResNet
    # ImageDataGenerator output [0-255], ResNet cần preprocessing đặc biệt
    ...
    print(f"✅ ResNet50 Model created for Keras 3.x (TF {tf.__version__})")
